...
void* func(int j)
{
    ...
    for(int i = 0; i < k; i++)
    {
        ...
        fprintf(fout, "Request number %i after %lu us by thread %i\n", i + 1, chrono::duration_cast<chrono::microseconds>(t1-t0).count(), j + 1);
        waiting[j] = true;
        key = 1;
        while (waiting[j] && key == 1)
        {
            key = atomic_compare_exchange_weak(&my_lock, &expected, 1);
        }
        waiting[j] = false;
        my_time t2 = chrono::steady_clock::now();
        fprintf(fout, "Entry number %i after %lu us by thread %i\n", i + 1, chrono::duration_cast<chrono::microseconds>(t2-t0).count(), j + 1);
        ...
        fprintf(fout, "Exit number %i after %lu us by thread %i\n", i + 1, chrono::duration_cast<chrono::microseconds>(t3-t0).count(), j + 1);
